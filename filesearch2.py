...
def findfile2(searchdir, searchext):
    ...
    try:
        ...
        print("Error - no permission to search: ", searchdir)
        return []
    ...
    # print(contents)
        ...
        print("Error - no read permission for: ", searchdir)
        os.chdir(camefrom)
        return []
...
def findfile3(searchdir, searchext):

    foundfiles = []

    for (currentpath, subdirs, files) in os.walk(searchdir, followlinks=True):
        for name in files:
            if name.endswith(searchext):
                foundfiles.append(os.path.join(currentpath, name))

    return foundfiles


# allfiles = findfile(os.path.abspath(SearchDir), SearchExt)
# for name in allfiles :
#     print(name)
allfiles = findfile2(os.path.abspath(SearchDir), SearchExt)
for name in allfiles:
    print(name)
allfiles = findfile3(os.path.abspath(SearchDir), SearchExt)
for name in allfiles:
    print(name)
